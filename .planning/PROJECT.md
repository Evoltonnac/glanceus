...
## 3. 当前状态 (v1.0 Shipped, v1.1 Planning)
...
3. OAuth（Authlib 架构）：
   - Authorization Code + PKCE：标准 OAuth 回归样本（GitHub/OpenRouter 类）。
   - Device Flow（RFC 8628）：Twitch 类设备授权场景（验证码 + 轮询）。
   - Client Credentials：服务到服务令牌获取与自动续期。
   - Callback 兼容：统一支持 `?code=` 与 `#access_token=`.
